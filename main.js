...
var ok = document.getElementById("onUpdate");
var cancel = document.getElementById("onCancel");
...
ok.onclick = function() {
  ...
  itemData = selInput;
  itemData.name = document.querySelector("#editInput").value;
 
  for(var i=0; i<itemList.children.length;i++){
    var addedAt = itemList.children[i].children[0].getAttribute("data-time");
    if(addedAt == itemData.addedAt){
      itemList.children[i].children[0].textContent = itemData.name;
  ...
  items.forEach((item) => {
    if (
      ...
      item.querySelector("[data-edit]").addEventListener("click", function (e) {
        e.preventDefault();
        // var popupData = prompt("update your task",itemData.name);
        modal.style.display = "block";
        document.querySelector("#editInput").value = itemData.name;
        selInput = itemData;
    ...
    todoItems.forEach((item) => {
      ...
      itemList.insertAdjacentHTML(
        ...
        `<li class="list-group-item d-flex justify-content-between align-items-center">
          ...
          <span>
              <a href="#" data-done><i class="bi ${text} green"></i></a>
